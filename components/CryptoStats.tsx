...
export default function CryptoStats() {
  ...
  useEffect(() => {
    async function fetchCryptoData() {
      // Don't attempt to fetch again if we've hit a rate limit recently (within last 30 seconds)
      const now = Date.now();
      if (rateLimited && now - lastFetchAttempt < 30000) {
        return;
      }
      ...
      try {
        ...
        const response = await fetch(
          `https://api.coingecko.com/api/v3/coins/markets?vs_currency=usd&ids=${popularCryptoIds.join(
            ","
        ...
        if (!stablecoinResponse.ok) {
          if (stablecoinResponse.status === 429) {
            setRateLimited(true);
          }
          throw new Error(
            `Failed to fetch stablecoin data: ${stablecoinResponse.status}`
          );
        }

        const stablecoinData = await stablecoinResponse.json();
        const usdcPrice =
          stablecoinData.find((coin: any) => coin.id === "usd-coin")
            ?.current_price || 1;
        const usdtPrice =
          stablecoinData.find((coin: any) => coin.id === "tether")
            ?.current_price || 1;
        ...
        // Different message for rate limiting
        if (rateLimited) {
          setError("CoinGecko API rate limit reached. Using available data.");
        } else {
    ...
    // Increase the interval if we've hit rate limits
    const intervalTime = rateLimited ? 300000 : 120000; // 5 minutes if rate limited, 2 minutes otherwise
    const intervalId = setInterval(fetchCryptoData, intervalTime);

    return () => clearInterval(intervalId);
  }, [cryptoData.length, popularCryptoIds, rateLimited, lastFetchAttempt]);
  ...
  return (
    <div id="market" className="py-16">
      <div className="text-center mb-10">
        <h2 className="text-3xl font-bold text-slate-800 mb-3 dark:text-gray-100">
          Market Overview
        </h2>
        <p className="text-slate-600 max-w-2xl mx-auto dark:text-gray-300">
          Stay updated with the latest cryptocurrency prices and trends
        </p>
      </div>
          ...
          <table className="min-w-full divide-y divide-purple-100 dark:divide-gray-800">
            <thead className="bg-gradient-to-r from-purple-50 to-white dark:from-gray-800 dark:to-gray-900">
              <tr>
                <th
                  scope="col"
                  className="px-6 py-5 text-left text-xs font-medium text-slate-500 uppercase tracking-wider dark:text-gray-400"
                >
                  Asset
                </th>
                <th
                  scope="col"
                  className="px-6 py-5 text-right text-xs font-medium text-slate-500 uppercase tracking-wider dark:text-gray-400"
                >
            ...
            <tbody className="bg-white divide-y divide-purple-100 dark:bg-gray-900 dark:divide-gray-800">
              {cryptoData.map((crypto, index) => (
                <tr
                  key={index}
                  className="hover:bg-purple-50 transition-colors duration-150 dark:hover:bg-gray-800"
                >
                    ...
                    <div className="flex items-center">
                      {crypto.image ? (
                        ...
                        <div className="w-10 h-10 flex-shrink-0 rounded-full bg-purple-100 flex items-center justify-center mr-3 dark:bg-gray-800">
                          <span className="font-bold text-purple-600 text-sm dark:text-purple-400">
                  ...
                  <td className="px-6 py-5 whitespace-nowrap text-right text-sm font-medium text-slate-900 dark:text-gray-100">
                    {getPriceSymbol()}{" "}
                    {getCurrentPrice(crypto).toLocaleString(undefined, {
        ...
        <div className="py-4 px-6 bg-gradient-to-r from-purple-50 to-white border-t border-purple-100 dark:from-gray-800 dark:to-gray-900 dark:border-gray-800">
          <p className="text-xs text-slate-500 text-center dark:text-gray-400">
            {isLoadingInitial ? (
              ...
              <span>
                <strong>Supported by CoinGecko</strong>{" "}
                <span className="text-gray-600 text-xs">
                  {" "}
